...
async def briefing_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            await update.message.reply_text(f"Morning briefing generated: {audio_url}")
    except Exception as e:
...
def _fetch_kuma_status():
    ...
    for monitor in monitors:
        ...
        try:
            # Uptime kuma get_heartbeats returns a list of dictionaries for the specified monitor
            heartbeats = api.get_heartbeats(m_id)
            if heartbeats and len(heartbeats) > 0:
                # Assuming the last heartbeat is the first or last in the list, typical format is dict or list
                # Actually get_heartbeats returns list of dicts. We take the latest (usually last item or depends on sorting).
                # To be safe, we check if any of the recent heartbeats shows UP. Let's just use the last one in the list.
                latest = heartbeats[-1]
